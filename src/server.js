  ...
  chainIndexer = new EvmIndexer({
    ...
    onIssueUpsert: (chainIssue) => {
      ...
      const issue = {
        ...
        kind: "issue",
        id: chainIssue.key,
        roomId: `job:${chainIssue.key}`,
        status: chainIssue.closed ? "closed" : chainIssue.canceled ? "closed" : "open",
        ...
        closedAt: chainIssue.closed || chainIssue.canceled ? existing?.closedAt ?? new Date().toISOString() : null,
        closedBy: chainIssue.closed || chainIssue.canceled ? chainIssue.opener : null,
        resolution: existing?.resolution ?? null,
        bounty: Number.parseInt(String(chainIssue.stableBounty ?? "0"), 10) || 0,
        metadataHash: chainIssue.metadataHash,
        chain: { kind: "evm", chainId: chainIssue.chainId, jobId: chainIssue.jobId },
        ...
        solver: chainIssue.winner ?? existing?.solver ?? null,
